...
public partial class JukeboxViewModel : ViewModelBase, IAsyncDisposable
{
    ...
    public string? InitialFile { get; set; } [ObservableProperty] private bool _stayOnTop;
        ...
        _initTask = Task.Run(() => 
        {
            ...
            Avalonia.Threading.Dispatcher.UIThread.Post(async () =>
            {
                ...
                if (!string.IsNullOrEmpty(InitialFile))
                {
                    ...
                    {
                        AppendToPlaylist(new[] { InitialFile });
                    }
                    else if (System.IO.Directory.Exists(InitialFile))
                    {
                        var ext = new[] { ".mp3", ".wav", ".flac", ".mp4", ".mkv", ".avi", ".webm", ".ogg" };
                        var paths = System.IO.Directory.EnumerateFiles(InitialFile, "*.*", System.IO.SearchOption.AllDirectories)
                            .Where(f => ext.Contains(System.IO.Path.GetExtension(f).ToLowerInvariant()))
                            .ToArray();
    ...
    public void SetProjectMControl(JukeboxVisualizations.Controls.ProjectMControl control)
    {
    ...
    public void LoadSystemLogo(string systemName)
    {
        try
        {
            var uri = new Uri($"avares://Jukebox/Assets/Systems/{systemName}.png");
    ...
    private void InitializeVideoVLC()
    {
        try
        {
            ...
            _videoMediaPlayer.Volume = InitialVolume;
    ...
    private void OnEndReached(object? sender, EventArgs e) 
    {
        Task.Run(async () => 
        {
            ...
            {
                if (IsLoopEnabled && Tracks.Count == 1)
                {
                    PlayCurrentTrack();
                }
                else
                {
                    GotoNextTrack();
                    if (CurrentIndex == 0 && !IsLoopEnabled)
                    {
