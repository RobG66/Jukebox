...
public partial class App : Application
{
    ...
    public override void OnFrameworkInitializationCompleted()
    {
        if (ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
            ...
            // ── Plugin loading ────────────────────────────────────────────
            // Scan the plugins/ folder for media browser plugins, load
            // each one, and populate PlaylistViewModel.MediaBrowserTabs.
            // This happens synchronously before the window is shown so
            // plugin destinations are present on first paint. If a plugin fails
            // to load, it's silently skipped with a debug log — the app
            // continues to start normally.
            Jukebox.Services.LoadedPlugins? loadedPlugins = null;
            try
            {
                ...
                foreach (var browser in loadedPlugins.MediaBrowsers)
                {
                    try
                    {
                        var view = browser.CreateView();
                        vm.PlaylistViewModel.MediaBrowserTabs.Add(new ViewModels.PluginTab
                        {
                            Browser = browser,
                            View = view
                        });
                        Console.WriteLine($"[Plugin] Added browser: {browser.DisplayName} ({browser.Id})");
                    }
                    catch (Exception ex)
                    {
                        var msg = $"[Plugin] {browser.Id}: CreateView failed: {ex.Message}";
            ...
            {
                var msg = $"[Plugin] Plugin loading failed: {ex.Message}";
                System.Diagnostics.Debug.WriteLine(msg);
                Console.WriteLine(msg);
            }

            // ── Visualizer plugin loading ──────────────────────────────────
            try
            {
                var visualizers = loadedPlugins?.Visualizers
                    ?? Array.Empty<Jukebox.Plugin.Abstractions.IJukeboxVisualizerPlugin>();
                Console.WriteLine($"[Plugin] Loader found {visualizers.Count} visualizer(s).");
                vm.VisualizerPlugins = visualizers;
                if (visualizers.Count > 0)
                {
                    vm.ActiveVisualizer = visualizers[0];
                }
            }
            catch (Exception ex)
            {
                var msg = $"[Plugin] Visualizer plugin loading failed: {ex.Message}";
                System.Diagnostics.Debug.WriteLine(msg);
                Console.WriteLine(msg);
            }
            // ─────────────────────────────────────────────────────────────
            ...
            if (desktop.Args != null)
            {
                foreach (var argRaw in desktop.Args)
                {
                    ...
                    if (a == "-?" || a == "-help" || a == "--help" || a == "?" || a == "/?" || a == "/help")
                    {
                        ...
                        Console.WriteLine(" -novisualizer           : Disable the active visualizer.");
