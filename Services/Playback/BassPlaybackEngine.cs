...
public sealed class BassPlaybackEngine : IMediaPlayerEngine
{
    ...
    public void Initialize()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                // this means the visualizer gets audio data in bursts: 5-6 frames with no new data,
    ...
    public async Task PlayAsync(JukeboxTrack track)
    {
        ...
        try
        {
            ...
            {
                bool isStaticFileUrl = false;
                try
                {
                    var uri = new Uri(urlToPlay);
                    var path = uri.AbsolutePath;
                    isStaticFileUrl = path.EndsWith(".mp3", StringComparison.OrdinalIgnoreCase) ||
