...
namespace Jukebox.Views
{
    public partial class ThreeButtonDialogView : Window
    {
        ...
        {
            var windowOwner = owner as Window;
            if (windowOwner == null)
            {
                var desktop = Avalonia.Application.Current?.ApplicationLifetime as IClassicDesktopStyleApplicationLifetime;
                if (desktop != null)
                {
                    windowOwner = desktop.Windows.FirstOrDefault(w => w.IsActive)
                                  ?? desktop.Windows.LastOrDefault(w => w.IsVisible && w is not ThreeButtonDialogView)
                                  ?? desktop.MainWindow;
                }
            }
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            if (e.Key == Key.Escape)
            {
                ...
                else if (Button3.IsVisible) Close(_button3Result);
                else Close(ThreeButtonResult.Button1);
        ...
        private void ApplyIconTheme(DialogIconTheme theme)
        {
            ...
            {
                DialogIconTheme.Warning => "avares://Jukebox/Assets/Icons/warning-triangle.png",
                DialogIconTheme.Info => "avares://Jukebox/Assets/Icons/info-circle.png",
                DialogIconTheme.Error => "avares://Jukebox/Assets/Icons/error-circle.png",
                _ => "avares://Jukebox/Assets/Icons/question-circle.png"
            };
