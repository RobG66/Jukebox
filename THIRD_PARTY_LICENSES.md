...
* **Redistribution:** Allowed, including in binary form, provided the
  ...
  release artifact produced by the Avalonia.ProjectM CI workflow.

### libvgm (VGM Player Core & Shim) — `vgm-player_Win64.dll` / `libvgm-player.so`

* **Website:** https://github.com/RobG66/libvgm (fork of Valley Bell's libvgm)
...
## Optional companion project — Avalonia.ProjectM

* **Source:** https://github.com/RobG66/Avalonia.ProjectM
* **License:** See that project's `THIRD_PARTY_LICENSES.md` for the
  licensing of `Avalonia.ProjectM.dll`, `libprojectM`, and GLEW.
* **Distribution model:** The external visualizer plugin loads the compiled
  `Avalonia.ProjectM.dll` from its own installed plugin directory. The managed wrapper, native libraries,
  license, presets, and textures remain together in that package.
